...
def main():
    ...
    model, diffusion = create_regressor_and_diffusion(in_channels = 1, regressor_depth=4,
    ...
    #     deterministic = False
    # )
    RegStruc = np.load('RegOutput.npy') # design variables 此处Input 与 output互换了，请关注数组维度以判断
    RegHomo = np.load('RegInput.npy') # material properties

    RegStruc = np.load('Output_37437.npy')
    RegHomo = np.load('Input_37437.npy')
    ...
    Homo_scaled = scaler_Homo.fit_transform(RegHomo) # fit - transform

    reversed_scale = scaler_Homo.inverse_transform(Homo_scaled)

    Homo_scaled = RegHomo[:,0]
    ...
    def forward_backward_log(Homo_scaled, prefix="train", print_res=False):
        ...
        batch_struc = np.tile(batch_struc, (6,))[:,:64]
